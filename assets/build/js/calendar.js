...
/* harmony import */ var _calendar_js_js_year_calendar_min__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./calendar/js/js-year-calendar.min */ "./src/js/calendar/js/js-year-calendar.min.js");
/* harmony import */ var _calendar_js_js_year_calendar_min__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_calendar_js_js_year_calendar_min__WEBPACK_IMPORTED_MODULE_1__);
...
var url_json = "".concat(actoutsData.root_url, "/wp-json/actouts/v1/event");
fetch(url_json, {
  method: "GET",
  headers: {
  ...
  function dispayElWithCommonDate() {
    var newArray = [];
    dataEvents.map(function (item, index) {
      var _ref;

      newArray.push("".concat(item.startDate, "-").concat(item.endDate));

      var getDates = function getDates(newDateS, endDateE) {
        var dates = [];
      ...
      var dates = getDates(item.startDate, item.endDate);
      newArray.push(dates);
      newArray = (_ref = []).concat.apply(_ref, _toConsumableArray(newArray)); /// function check Duplicate dates
      ...
      var duplicateDate = checkDuplicate(newArray);
      duplicateDate.map(function (item, index) {
        return object = {
          item: item
        };
      });
      ...
      if (item.priority > 0) {
        ...
        var month = item.startDate.getMonth();
        var getMonths = document.querySelector('.months-container');
        var monthEl = getMonths.getElementsByClassName('month-container')[month];
        var findDay = monthEl.querySelectorAll(".day-content");
        var allDatesBetween = [];
        datesBetween = getDates(item.startDate, item.endDate);
        allDatesBetween.push(datesBetween);
        allDatesBetween = (_ref2 = []).concat.apply(_ref2, _toConsumableArray(allDatesBetween));
        var single = [];

        for (var k = 0; k < allDatesBetween.length; k++) {
          single = allDatesBetween[k].getDate();

          for (var f = 0; f < findDay.length; f++) {
            if (findDay[f].innerHTML == single) {
              findDay[f].style.backgroundColor = "".concat(item.color);
            }
          }
        }
      }
...
/******/ 	__webpack_require__("./node_modules/element-remove/index.js");
